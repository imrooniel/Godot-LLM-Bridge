...
`bridge/cli/editor_bridge_cli.py` under the `game` tree. Use this when the thing you care about
...
```bash
python3 bridge/cli/editor_bridge_cli.py ping            # editor alive?
python3 bridge/cli/editor_bridge_cli.py game ping       # game running + bridge reachable?
```
...
```bash
python3 bridge/cli/editor_bridge_cli.py game hold W        # press AND hold (marks move_forward held)
python3 bridge/cli/editor_bridge_cli.py game held-keys     # confirm W / move_forward is held
# ... let it run, sample ...
python3 bridge/cli/editor_bridge_cli.py game release W     # ALWAYS release when done
```
...
python3 bridge/cli/editor_bridge_cli.py game watch --nodes Main/Player \
    --props '["global_position","rotation.y"]' --hz 30
sleep 2
# Read the last 3 samples (should show position changing while a key is held)
python3 bridge/cli/editor_bridge_cli.py game sample --limit 3
# Stop when done
python3 bridge/cli/editor_bridge_cli.py game telemetry-stop
```
...
```bash
python3 bridge/cli/editor_bridge_cli.py game get-many Main/Player Main/Player \
