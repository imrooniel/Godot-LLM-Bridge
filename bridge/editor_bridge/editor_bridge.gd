...
const BridgeContextScript := preload("res://bridge/editor_bridge/bridge_context.gd")
const DapSessionScript := preload("res://bridge/editor_bridge/dap_session.gd")
const GameProxyScript := preload("res://bridge/editor_bridge/game_proxy.gd")
const LogCollectorScript := preload("res://bridge/editor_bridge/log_collector.gd")
...
const COMMANDS_DIR := "res://bridge/editor_bridge/commands"
...
# Deferred gd-eval compile-error responses: request id ->
# {deadline, url, base} — answered once the panel shows the exact parse
# error (BD-008) or on deadline with the fallback text.
var _pending_eval_errors: Dictionary = {}
const PARSE_ERROR_WAIT_MS := 2500
...
func _enter_tree() -> void:
	...
	_dap.breaked_changed.connect(_on_dap_breaked_changed)
	_ctx.dap = _dap
...
## Compile a throwaway wrapper around the expression in the EDITOR.
## Returns {} when it compiles; on failure a Dictionary with:
##   base — fallback error text; url — the gdscript://-<id> of the failed
##   temp script (from the log collector) so the panel scan can find the
##   exact "Parse Error: …" line the engine prints one tick later (BD-008).
func _compile_eval_check(source: String) -> Dictionary:
	var lines := PackedStringArray()
	for setting: Dictionary in ProjectSettings.get_property_list():
		var sname := str(setting.get("name", ""))
		if sname.begins_with("autoload/"):
			var aname := sname.trim_prefix("autoload/")
			if aname.is_valid_identifier():
				lines.append("var %s" % aname)
	lines.append("func _ev() -> Variant:")
	lines.append("\treturn (%s)" % source)
	var check := GDScript.new()
	check.source_code = "\n".join(lines) + "\n"
	if check.reload() == OK:
		return {}
	return {"base": "gd-eval: expression does not compile (game not touched)",
			"url": _recent_failed_check_url()}


## The numeric id of the most recent failed temp-script compile, taken from
## the editor log collector's "ERROR at GDScript::reload (gdscript://<id>.gd:…)"
## entry (synchronous — logged during the reload() that just failed).
func _recent_failed_check_url() -> String:
	if _log_collector == null:
		return ""
	var messages := _log_collector.get_messages()
	for i in range(messages.size() - 1, -1, -1):
		var line := str(messages[i])
		var marker := "GDScript::reload (gdscript://"
		var idx := line.find(marker)
		if idx != -1:
			var rest := line.substr(idx + marker.length())
			var dot := rest.find(".")
			return rest.substr(0, dot) if dot != -1 else ""
	return ""


## Deferred BD-008 responses: once the Output panel shows the exact
## "Parse Error:" line for our failed temp script, answer with it; on
## deadline, answer with the base message.
func _tick_pending_eval_errors() -> void:
	if _pending_eval_errors.is_empty():
		return
	var text := _panel_text()
	var now := Time.get_ticks_msec()
	for id: String in _pending_eval_errors.keys().duplicate():
		var p: Dictionary = _pending_eval_errors[id]
		var detail := _find_parse_detail(text, str(p.get("url", "")))
		if detail != "":
			_pending_eval_errors.erase(id)
			_write_response(id, _error(-32602,
					"gd-eval: expression does not compile — " + detail + " (game not touched)"))
			continue
		if now >= int(p.get("deadline", 0)):
			_pending_eval_errors.erase(id)
			var base := str(p.get("base", "gd-eval: expression does not compile"))
			_write_response(id, _error(-32602,
					base + "; exact parser message in editor Output"))


func _find_parse_detail(panel_text: String, url_id: String) -> String:
	if url_id.is_empty() or panel_text.is_empty():
		return ""
	var lines := panel_text.split("\n")
	for i in range(lines.size() - 1, -1, -1):
		var line := str(lines[i]).strip_edges()
		if line.contains(url_id) and line.contains("Parse Error:"):
			var detail := line.substr(line.find("Parse Error:") + len("Parse Error:")).strip_edges()
...
func _proxy_game(id: String, cmd: String, args: Dictionary, timeout: float) -> Variant:
	...
	if method == "gd_eval" or method == "eval":
		# Pre-validate in the EDITOR: a failed GDScript compile inside a
		# debug-launched game triggers Break-on-Error and wedges the game
		# mid-request (dogfood BD-004). Here a compile error is just a log line.
		var source := str(args.get("expression", ""))
		var policy := EvalPolicyScript.validate(source)
		if policy != "":
			return _error(-32602, "gd-eval: " + policy)
		var check := _compile_eval_check(source)
		if not check.is_empty():
			# Defer: the exact "Parse Error:" line appears in the Output panel
			# one poll tick after reload() failed (BD-008).
			_pending_eval_errors[id] = {
				"deadline": Time.get_ticks_msec() + PARSE_ERROR_WAIT_MS,
				"url": str(check.get("url", "")),
				"base": str(check.get("base", "gd-eval: expression does not compile")),
			}
			return null
...
func _update_state() -> void:
	...
	_ctx.state = {
		"editor": {
			...
			# Project-root-relative (the CLI's CWD). Absolute paths here would
			# leak machine-specific dirs into state.json and break after a
			# folder rename/clone on another machine (dogfood BD-003).
			"project_path": ".",
...
## §6 legibility contract — REUSE scripts/tools/bridge_contract.gd (Task 1) so the
## editor and the CLI share one definition of the response shape.

const BridgeContractScript := preload("res://bridge/contract/bridge_contract.gd")

## Editor-side state object: {game, breaked, turn}. game_status is a Dictionary
## (the helper reads .get("running", false)); breaked comes from the DAP session.
## turn is deliberately an empty {} — the bridge cannot know LLM/human turn
## ownership; the harness owns that (spec §3).
func _contract_state() -> Dictionary:
	var ei := get_editor_interface()
	var playing: bool = ei.is_playing_scene() if ei else false
	var breaked: bool = _dap.breaked if _dap else false
	return BridgeContractScript.state_object(true, {"running": playing}, breaked, {})

## Wrap a command payload into the contract. A payload that carries `error` (a
## command's make_error) is a failure; otherwise success. The payload's own `ok`
## is consumed as the success/failure signal and dropped from the wrapped result.
func _wrap_contract(result: Dictionary) -> Dictionary:
	var st := _contract_state()
	var err_raw: Variant = result.get("error")
	if err_raw is Dictionary:
		var e: Dictionary = (err_raw as Dictionary).duplicate(true)
		if not e.has("reason"):
			e["reason"] = "editor_error"
		if not e.has("suggested"):
			e["suggested"] = []
		return BridgeContractScript.err(int(e.get("code", -32000)), str(e.get("message", "")),
			str(e.get("reason", "editor_error")), e.get("suggested", []), st)
	if err_raw != null:
		# Malformed command put a non-Dictionary error value; wrap it defensively.
		return BridgeContractScript.err(-32000, str(err_raw), "editor_error", [], st)
	return BridgeContractScript.ok(result, st)


func _write_response(id: String, response: Dictionary) -> void:
	if not response.has("id"):
		response["id"] = id
	var wrapped: Dictionary = _wrap_contract(response)
	_atomic_write("%s/%s.json" % [OUTBOX_DIR, id], JSON.stringify(wrapped, "", false))
...
## DAP break state changed — mirror it into the game proxy so a `game.*` WRITE
## issued while the game is paused at a breakpoint is deferred (a legible
## `game_paused` response) instead of hanging to the proxy timeout and wedging
## the single-client slot (spec §4.3, BD-012 DAP-break variant).
func _on_dap_breaked_changed(b: bool) -> void:
	if _game_proxy:
		_game_proxy.set_breaked(b)
